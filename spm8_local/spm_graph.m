...
switch Cplot
    case {'Contrast estimates and 90% C.I.','Fitted responses','Parametric responses'}
        AddMean = spm_input('Add mean to plot?',+1,'y/n',[1 0],2);
    otherwise 
        AddMean = 0;
end
...
switch Cplot
    ...
    case 'Contrast estimates and 90% C.I.'
        ...
        for j = 1:length(cbeta)
            line([j j],([CI(j) 0 - CI(j)] + cbeta(j) + yM),...
                'LineWidth',6,'Color',Col(3,:))
        end

        title(TITLE,'FontSize',12)
        xlabel('contrast')
        ylabel(['contrast estimate',yMstr,XYZstr])
    ...
    case 'Fitted responses'
        ...
        [p q] = sort(x);
        if all(diff(x(q)))
            plot(x(q),Y(q)+yM,'LineWidth',4,'Color',Col(2,:));
            plot(x(q),y(q)+yM,':','Color',Col(1,:));
            plot(x(q),y(q)+yM,'.','MarkerSize',8, 'Color',Col(3,:));

        else
            plot(x(q),Y(q)+yM,'.','MarkerSize',16,'Color',Col(1,:));
            plot(x(q),y(q)+yM,'.','MarkerSize',8, 'Color',Col(2,:));
            xlim = get(gca,'XLim');
    ...
    case 'Event-related responses'
        ...
        switch TITLE
            ...
            case 'fitted response and PSTH'
                %----------------------------------------------------------
                errorbar(PST,PSTH+yM,PCI)
                plot(PST,PSTH+yM,'LineWidth',4,'Color',Col(2,:))
                plot(x,Y+yM,'-.','Color',Col(3,:))

            case 'fitted response and 90% C.I.'
                %----------------------------------------------------------
                plot(x,Y+yM,'Color',Col(2,:),'LineWidth',4)
                plot(x,Y + CI+yM,'-.',x,Y - CI,'-.','Color',Col(1,:))

            case 'fitted response and adjusted data'
                %----------------------------------------------------------
                plot(x,Y+yM,'Color',Col(2,:),'LineWidth',4)
                plot(pst,y+yM,'.','Color',Col(3,:))
